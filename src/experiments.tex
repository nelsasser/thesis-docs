...
As is common in energy price forecasting literature, and time-series forecasting literature in general, we first
...
We sweep over multiple values of $T < 365$ and compare against a baseline that is never recalibrated
...
value to be a good tradeoff between performance and computation time when running experiment back-tests.

\begin{table}[htb]
    \caption[Results of parameter recalibration]{
        Evaluation metrics comparing various parameter recalibration intervals.
    }
    \begin{center}
...
again time-series forecasting literature in general, recent data is more informative to upcoming forecasts than data
...
training window.
This is likely caused by the models becoming over-parameterized and overfitting on the reduced amounts of training data.

\begin{table}[htb]
    \caption[Results of reducing rolling training window size]{
        Evaluation metrics on reduced rolling training window sizes.
    }
    \begin{center}
        \begin{tabular}{||c|c|c|c||} \hline
